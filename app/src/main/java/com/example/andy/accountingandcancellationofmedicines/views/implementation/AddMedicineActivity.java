...
public class AddMedicineActivity extends AppCompatActivity implements AddMedicineImpl {
    ...
    @ViewById(R.id.listShops)
    ListView mListView;
    @ViewById(R.id.editTextNameMedicine)
    EditText mNameMedicine;
    @ViewById(R.id.editTextNote)
    EditText mNote;
    @ViewById(R.id.editTextAD)
    EditText mArrivalDate;
    @ViewById(R.id.Amount)
    EditText mAmount;
    @ViewById(R.id.DateOfManufacture)
    EditText mDateOfManufacture;
    @ViewById(R.id.shelfLife)
    EditText mShelfLife;
    @ViewById(R.id.MeasurAmount)
    NiceSpinner mMeasureSpinner;
    @ViewById(R.id.spinnerLotNumber)
    NiceSpinner mLotNumberSpinner;
    @ViewById(R.id.add_medicine_pageAdd)
    Button mAddButton;
    ...
    public void setTextForUpdate() {
        ...
        for (int i = 0; i < mMeasureEntityList.size(); i++) {
            if (mMeasureEntityList.get(i).getIdMeasure().equals(mMedicineEntity.getIdMeasure())) {
                mMeasureSpinner.setSelectedIndex(i);
                break;
            } else
                mMeasureSpinner.setSelectedIndex(0);
        }
    }
    ...
    public void initComponent() {
        try {
            ...
            mListView.setAdapter(new ShopAdapter(this, shopEntityArrayList));
            mLotNumberSpinner.setTextColor(Color.BLACK);
            mMeasureSpinner.setTextColor(Color.BLACK);
    ...
    public void setDataFromDatabase() {
        ...
        mMedicineEntity.setIdMeasure(mMeasureEntityList.get(mMeasureSpinner.getSelectedIndex()).getIdMeasure());
